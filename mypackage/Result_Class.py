...
class Result:
    ...
    def _calculate_statistics(self) -> dict:
        ...
        total_return = (1 + self.returns).prod() - 1  # Performance totale
        annual_return = (
            ((1 + total_return) ** (252 / len(self.returns))) - 1
            if len(self.returns) > 0
            else pd.Series(0, index=self.returns.columns)
        )  # Performance annualisée
        volatility = self.returns.std() * np.sqrt(252)  # Volatilité annualisée

        # Ratio de Sharpe
        sharpe_ratio = pd.Series(
            [an_return / vol if vol != 0 else 0 for an_return, vol in zip(annual_return, volatility)],
        ...
        downside_deviation = (
            np.sqrt((downside_returns ** 2).mean()) * np.sqrt(252) if not downside_returns.empty else 0
        )
        sortino_ratio = pd.Series(
            [an_return / downside_deviation if downside_deviation != 0 else np.nan for an_return in annual_return],
            index=annual_return.index,
        )
        ...
        cvar_95 = pd.Series(
            [
                self.returns[asset][self.returns[asset] <= var_95[asset]].mean()
                if not self.returns[asset][self.returns[asset] <= var_95[asset]].empty
                else 0
        ...
        profit_loss_ratio = pd.Series(
            [abs(avg_g / avg_l) if avg_l != 0 else np.nan for avg_g, avg_l in zip(avg_gain, avg_loss)],
            index=avg_gain.index,
        )

        # Pourcentage de trades gagnants
        win_rate = self.returns.apply(
            lambda col: col[col > 0].count() / col[col != 0].count() if col[col != 0].count() > 0 else 0
        )

        # Facteur de profitabilité
        profit_factor = self.returns.apply(
            lambda col: abs(col[col > 0].sum() / col[col < 0].sum()) if col[col < 0].sum() != 0 else 1
        )
    ...
    def plot(self, name_strat: str, backend: str = 'matplotlib', include_costs: bool = True):
        ...
        if backend == 'matplotlib':
            ...
            fig = go.Figure()
            fig.add_trace(
                go.Scatter(x=cumulative_returns.index, y=cumulative_returns.values, name=f'Rendements cumulatifs {titre} coûts inclus')
            )
...
def compare_results(results: dict, backend: str = 'matplotlib'):
    ...
    if backend == 'matplotlib':
        stats_comparison.plot(kind='bar', figsize=(12, 6), title='Comparaison des stratégies')
        plt.tight_layout()

    elif backend == 'seaborn':
        plt.figure(figsize=(12, 6))
        sns.barplot(data=stats_comparison.reset_index().melt(id_vars='index'), x='index', y='value', hue='variable')
